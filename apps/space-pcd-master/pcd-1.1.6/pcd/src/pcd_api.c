...
{
    /* Init IPC */
    if ( IPC_init( 0 ) != (IPC_status_e)PCD_STATUS_OK )
    {
    ...
    /* Start IPC */
    if ( IPC_start( CONFIG_PCD_SERVER_NAME, &pcdContext, 0 ) != (IPC_status_e)PCD_STATUS_OK )
    {
    ...
    /* Set owner */
    if ( IPC_set_owner( pcdContext, CONFIG_PCD_OWNER_ID ) != (IPC_status_e)PCD_STATUS_OK )
    {
        IPC_stop( pcdContext );
...
PCD_status_e PCD_api_check_messages( void )
{
    ...
    while ( budget-- && IPC_wait_msg( pcdContext, &msg, IPC_TIMEOUT_IMMEDIATE ) == 0 )
    {
        ...
        if ( IPC_get_msg_context( msg, &msgContext ) == (IPC_status_e)PCD_STATUS_OK )
        {
        ...
        if ( replyMsg )
        {
            ...
            /* Send response */
            if ( IPC_reply_msg( msg, replyMsg ) != (IPC_status_e)PCD_STATUS_OK )
            {
                IPC_free_msg( replyMsg );
            }
